...
export async function generateStaticParams() {
  return MOCK_BARBER.map((service) => ({
    id: service.id,
  }));
}

export default async function BarberDetailsPage({ params }: { params: { id: string } }) {
  const resolvedParams = await params;
  const service = MOCK_BARBER.find((s) => s.id === resolvedParams.id);
  ...
  const otherServices = MOCK_BARBER;

  return (
    <div className="bg-[#fcfcff] min-h-[calc(100vh-80px)] pb-10 text-ink">
      <div className="max-w-7xl mx-auto">
        <div className="py-4 text-[1rem] font-medium">
          <Link href="/" className="text-purple font-bold hover:underline cursor-pointer">Home</Link> › <Link href="/browse/barber" className="text-purple font-bold hover:underline cursor-pointer">Barber</Link> › <span className="text-muted">{service.name}</span>
        </div>

        <div className="mt-2 mb-8">
          <div className="text-[#8b8693] font-bold text-[0.8rem] tracking-[0.14em] uppercase mb-1.5">Reserve and confirm</div>
          <h2 className="font-sans font-bold leading-[1.12] tracking-[-0.02em] text-[clamp(1.6rem,3vw,2.2rem)] mb-2">Book a hair appointment</h2>
          <p className="text-muted max-w-105">Fresh cuts, fades, and beard work from trusted barbers near you. Choose a service and a time that suits you.</p>
        </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4.5 overflow-x-auto pb-2">
              {otherServices.slice(0, 5).map((s, i) => (
                <Link href={`/browse/barber/${s.id}`} key={i} className="relative w-17.5 h-17.5 rounded-xl overflow-hidden shrink-0 border-2 border-transparent hover:border-purple transition-colors">
            ...
            <div className="flex flex-col gap-2.5">
              {otherServices.map((s) => (
                <Link href={`/browse/barber/${s.id}`} key={s.id} className={`flex items-center justify-between p-[13px_16px] border ${service.id === s.id ? 'border-purple bg-[#faf7ff]' : 'border-line bg-white'} rounded-[13px] hover:border-purple transition-colors cursor-pointer`}>
                  ...
                  <b className="font-extrabold text-[1.05rem]">USD 45</b>
                </Link>
              ))}
            </div>
