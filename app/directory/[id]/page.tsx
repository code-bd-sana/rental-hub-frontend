    ...
    <div className='min-h-screen bg-[#f8fafc] pb-20'>
      ...
      <div className='relative w-full h-[40vh] min-h-[300px]'>
        ...
        <div className='absolute inset-0 flex items-center justify-center'>
          <div className='text-center px-4 max-w-4xl mx-auto'>
            <h1 className='text-4xl sm:text-5xl font-bold text-white mb-4 drop-shadow-lg'>{listing.title}</h1>
      ...
      <div className='max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        <div className='text-sm text-gray-500 mb-8 flex items-center gap-2'>
          ...
          <span>›</span>
          <Link href='/directory?category=FOOD' className='hover:text-gray-900 transition-colors'>Food</Link>
          <span>›</span>
          <span className='font-medium text-gray-900 truncate'>{listing.title}</span>
        </div>
          ...
          <div className='flex-1'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
              {listing.foodDetails?.items?.map((item: any) => (
                <div key={item.id} className='flex gap-4 bg-white border border-[#e7e1d6] rounded-2xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)]'>
                  <div className='relative w-[98px] h-[98px] flex-none rounded-xl overflow-hidden bg-gray-100'>
                    {item.imageUrl ? (
                      <Image src={item.imageUrl} alt={item.name} fill sizes="98px" className='object-cover' />
                    ) : (
                  ...
                  <div className='flex flex-col flex-1 min-w-0 py-1'>
                    <div className='font-bold text-[#15201f] text-[1.05rem] truncate'>{item.name}</div>
              ...
              <div className='p-5'>
                  ...
                  <div className='flex flex-col gap-3 mb-4 max-h-[300px] overflow-y-auto no-scrollbar'>
                    {cart.map((item, idx) => (
                      <div key={idx} className='flex justify-between items-center text-[15px]'>
                        ...
                        <span className='font-bold text-[#15201f]'>${(item.price * item.qty).toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {cart.length > 0 && (
                  <div className='flex justify-between items-center border-t border-gray-200 pt-4 mb-4'>
                    ...
                    <span className='font-extrabold text-xl text-[#15201f]'>${cartTotal.toFixed(2)}</span>
                  </div>
                )}
                ...
                <div className='flex flex-col gap-2.5'>
                  <button className='w-full bg-[#15201f] text-white py-2.5 rounded-xl font-bold text-[14px] hover:bg-black transition-colors flex items-center justify-center gap-2'>
                    <svg viewBox='0 0 24 24' fill='none' className='w-4 h-4'>
                      <path d='M21 12a8 8 0 0 1-11.5 7.2L4 20l.9-5A8 8 0 1 1 21 12Z' stroke='#fff' strokeWidth='2' strokeLinejoin='round'/>
                    </svg>
                    Request to chat
                  </button>
