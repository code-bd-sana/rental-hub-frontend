...
export function HostListings() {
  const [listings, setListings] = useState([
    { id: 'LST-1', title: 'Ocho Rios Grill', category: 'Restaurant', country: 'Jamaica', status: 'Live', hours: 'Daily 12pm to 11pm', phone: '+1 876 000 0004' },
    { id: 'LST-2', title: 'Ocho Rios Grill Catering', category: 'Restaurant', country: 'Jamaica', status: 'Pending review', hours: 'Mon to Fri 9am to 5pm', phone: '+1 876 000 0004' },
  ]);
  ...
  // Form state
  const [formData, setFormData] = useState({ title: '', category: 'Stay', country: 'Jamaica', hours: '', phone: '', status: 'Pending review' });

  const handleOpenAdd = () => {
    setFormData({ title: '', category: 'Stay', country: 'Jamaica', hours: '', phone: '', status: 'Pending review' });
    setIsAdding(true);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (editListing) {
      setListings(listings.map(l => l.id === editListing.id ? { ...formData, id: editListing.id } : l));
      setEditListing(null);
    } else {
    ...
    <div className="animate-in fade-in duration-300 relative">
      ...
      <div className="bg-white border border-[#e7e1d6] rounded-2xl shadow-[0_10px_30px_rgba(11,79,74,0.1)] overflow-hidden mt-4">
        <div className="flex justify-between items-center px-4 py-3.5 border-b border-[#e7e1d6]">
          ...
          <button onClick={handleOpenAdd} className="border border-[#2563eb] bg-[#2563eb] text-white rounded-[9px] px-3.5 py-1.75 text-[13px] font-semibold transition-colors hover:bg-[#1e40af] shadow-sm cursor-pointer">
            Add a listing
          </button>
        </div>
          ...
          <table className="w-full border-collapse text-[14px] whitespace-nowrap">
            <thead>
              <tr>
                ...
                <th className="text-left text-[#6b7b79] text-[11px] uppercase tracking-[0.5px] p-[10px_16px] border-b border-[#e7e1d6]">Location</th>
                <th className="text-left text-[#6b7b79] text-[11px] uppercase tracking-[0.5px] p-[10px_16px] border-b border-[#e7e1d6]">Hours & Contact</th>
                <th className="text-left text-[#6b7b79] text-[11px] uppercase tracking-[0.5px] p-[10px_16px] border-b border-[#e7e1d6]">Status</th>
                <th className="text-center text-[#6b7b79] text-[11px] uppercase tracking-[0.5px] p-[10px_16px] border-b border-[#e7e1d6]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {listings.map((lst) => (
                <tr key={lst.id} className="hover:bg-[#f8fafc] transition-colors">
                  ...
                  <td className="p-[12px_16px] border-b border-[#e7e1d6] text-[#6b7b79]">{lst.country}</td>
                  <td className="p-[12px_16px] border-b border-[#e7e1d6]">
                    <div className="text-[#15201f]">{lst.hours}</div>
                    <div className="text-[12px] text-[#6b7b79]">{lst.phone}</div>
                  </td>
                  <td className="p-[12px_16px] border-b border-[#e7e1d6]">
                    {lst.status === 'Live' && <span className="bg-[#dff3ec] text-[#1e9e72] text-[11px] font-bold px-2.25 py-0.75 rounded-[20px] uppercase tracking-[0.5px]">Live</span>}
                    {lst.status === 'Pending review' && <span className="bg-[#e6eefb] text-[#2a5db0] text-[11px] font-bold px-2.25 py-0.75 rounded-[20px] uppercase tracking-[0.5px]">Pending review</span>}
                    {lst.status === 'Draft' && <span className="bg-[#f1f5f9] text-[#64748b] text-[11px] font-bold px-2.25 py-0.75 rounded-[20px] uppercase tracking-[0.5px]">Draft</span>}
                  </td>
                  <td className="p-[12px_16px] border-b border-[#e7e1d6]">
                    <div className="flex items-center justify-center gap-3">
                      <button onClick={() => setViewListing(lst)} className="text-[#6b7b79] hover:text-[#2563eb] transition-colors cursor-pointer" title="View Preview">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z" /><circle cx="12" cy="12" r="3" /></svg>
                      </button>
                      <button onClick={() => handleOpenEdit(lst)} className="text-[#6b7b79] hover:text-[#1e9e72] transition-colors cursor-pointer" title="Edit">
                      ...
                      <button onClick={() => setDeleteListingId(lst.id)} className="text-[#6b7b79] hover:text-[#dc2626] transition-colors cursor-pointer" title="Delete">
              ...
              {listings.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-[#6b7b79]">No listings found. Add one to get started.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-[rgba(21,32,31,0.5)] z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-[#e7e1d6] flex justify-between items-center bg-[#f8fafc]">
              <h3 className="font-bold text-[18px] text-[#15201f]" style={{ fontFamily: '"Georgia", "Times New Roman", serif' }}>
                {editListing ? 'Edit Listing' : 'Add New Listing'}
              </h3>
              <button onClick={() => { setIsAdding(false); setEditListing(null); }} className="text-[#6b7b79] hover:text-[#15201f] text-2xl leading-none cursor-pointer">&times;</button>
            </div>
            <form onSubmit={handleSave}>
              <div className="p-6 space-y-4 text-[14px] max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[12px] font-bold text-[#15201f] mb-1.5">Listing Title</label>
                    <input required value={formData.title} onChange={e => handleFormChange('title', e.target.value)} placeholder="e.g. Blue Mountain Villa" className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] focus:outline-none focus:border-[#2563eb] transition-colors" />
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[12px] font-bold text-[#15201f] mb-1.5">Category</label>
                    <select value={formData.category} onChange={e => handleFormChange('category', e.target.value)} className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] bg-white focus:outline-none focus:border-[#2563eb] transition-colors">
                ...
                <div>
                  ...
                  <select value={formData.country} onChange={e => handleFormChange('country', e.target.value)} className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] bg-white focus:outline-none focus:border-[#2563eb] transition-colors">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12px] font-bold text-[#15201f] mb-1.5">Hours / Check-in</label>
                    <input required value={formData.hours} onChange={e => handleFormChange('hours', e.target.value)} placeholder="e.g. Daily 9am to 5pm" className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] focus:outline-none focus:border-[#2563eb] transition-colors" />
                  </div>
                  <div>
                    <label className="block text-[12px] font-bold text-[#15201f] mb-1.5">Phone Number</label>
                    <input required value={formData.phone} onChange={e => handleFormChange('phone', e.target.value)} placeholder="+1 800 000 0000" className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] focus:outline-none focus:border-[#2563eb] transition-colors" />
                  </div>
                </div>
                <div>
                  <label className="block text-[12px] font-bold text-[#15201f] mb-1.5">Status</label>
                  <select value={formData.status} onChange={e => handleFormChange('status', e.target.value)} className="w-full border border-[#e7e1d6] rounded-xl px-4 py-2.5 text-[14px] bg-white focus:outline-none focus:border-[#2563eb] transition-colors">
              ...
              <div className="p-5 border-t border-[#e7e1d6] bg-[#f8fafc] flex justify-end gap-3">
                <button type="button" onClick={() => { setIsAdding(false); setEditListing(null); }} className="px-5 py-2.5 rounded-xl font-bold text-[14px] text-[#6b7b79] hover:bg-[#e7e1d6] transition-colors cursor-pointer">Cancel</button>
                <button type="submit" className="bg-[#2563eb] text-white px-6 py-2.5 rounded-xl font-bold text-[14px] hover:bg-[#1e40af] transition-colors shadow-sm cursor-pointer">Save Listing</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {viewListing && (
        <div className="fixed inset-0 bg-[rgba(21,32,31,0.5)] z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-sm w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-[#e7e1d6] flex justify-between items-center bg-[#f8fafc]">
              <h3 className="font-bold text-[18px] text-[#15201f]" style={{ fontFamily: '"Georgia", "Times New Roman", serif' }}>Preview</h3>
              <button onClick={() => setViewListing(null)} className="text-[#6b7b79] hover:text-[#15201f] text-2xl leading-none cursor-pointer">&times;</button>
            </div>
            ...
            <div className="p-5 border-t border-[#e7e1d6] bg-white text-right">
              <button onClick={() => setViewListing(null)} className="bg-[#172554] text-white px-6 py-2.5 rounded-xl font-bold text-[14px] hover:bg-[#0f172a] transition-colors cursor-pointer shadow-sm">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className='fixed inset-0 bg-[rgba(21,32,31,0.5)] z-[100] flex items-center justify-center p-4 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl max-w-sm w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-200'>
            <div className='p-6 text-center'>
              ...
              <h3 className='font-bold text-[20px] text-[#15201f] mb-2' style={{ fontFamily: '"Georgia", "Times New Roman", serif' }}>
                Delete Listing
              </h3>
              <p className='text-[14px] text-[#6b7b79] leading-relaxed'>
                Are you sure you want to delete this listing? This action cannot be undone and will remove it from the directory immediately.
              </p>
            </div>
            <div className='p-5 border-t border-[#e7e1d6] bg-[#f8fafc] flex justify-center gap-3'>
              <button onClick={() => setDeleteListingId(null)} className='px-6 py-2.5 rounded-xl font-bold text-[14px] text-[#6b7b79] hover:bg-[#e7e1d6] transition-colors cursor-pointer w-full'>
                Cancel
              </button>
              <button onClick={confirmDelete} className='bg-[#ef4444] text-white px-6 py-2.5 rounded-xl font-bold text-[14px] hover:bg-[#dc2626] transition-colors shadow-sm cursor-pointer w-full'>
                Delete
              </button>
            </div>
          </div>
