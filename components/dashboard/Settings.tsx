  ...
  const fetchProfile = async () => {
    try {
      ...
      if (user) {
        ...
        setPhone(user.phone || '');
        if (user.role) {
          if (user.role === ROLES.SUPER_ADMIN) setRole('Administrator');
          else if (user.role === ROLES.LOADER) setRole('Content Loader');
          else if (user.role === ROLES.AGENT) setRole('Agent');
          else if (user.role === ROLES.HOST) setRole('Host');
          else if (user.role === ROLES.GUEST) setRole('Guest');
        }
      }
    } catch (error) {
    ...
    <div className='animate-in fade-in duration-300 max-w-4xl pb-10'>
      ...
      <form onSubmit={handleSaveAll} className='space-y-6'>
        ...
        <div className='bg-white rounded-2xl border border-[#e7e1d6] shadow-sm p-8'>
          <h3
            className='text-[18px] font-bold text-[#172554] mb-4'
            style={{ fontFamily: '"Georgia", "Times New Roman", serif' }}
          >
            Personal Details
          </h3>
