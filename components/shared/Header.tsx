  ...
  return (
    <>
      <header className='sticky top-0 z-50 bg-[rgba(251,248,255,.86)] backdrop-blur-[14px] border-b border-(--line)'>
        <div className='max-w-7xl mx-auto flex items-center gap-8 h-20 px-4 sm:px-6 lg:px-8'>
          ...
          <nav className='flex gap-3 ml-auto max-[900px]:hidden'>
            ...
            <Link href='/countries' className={navClass(pathname.startsWith('/countries'))}>
              Countries
            </Link>
      ...
      {mobileMenuOpen && (
        <div className='fixed top-18 left-0 right-0 z-49 bg-white border-b border-(--line) shadow-custom p-[10px_14px] block'>
          ...
          {isAuthenticated ? (
            ...
            <Link href='/login' onClick={() => setMobileMenuOpen(false)} className={mobileNavClass}>
              Sign In
            </Link>
          )}
        </div>
